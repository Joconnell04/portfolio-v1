...
export function ProjectsSection({ projects }: { projects: PortfolioProject[] }) {
  return (
    ...
    >
      <motion.div variants={fadeUpItem} className="grid gap-6 border-t border-white/10 pt-8 lg:grid-cols-[280px_minmax(0,1fr)]">
        <div className="space-y-3 sm:space-y-4">
          <TextScramble text="Selected projects" className="text-xs uppercase tracking-[0.3em] text-emerald-300/70" />
          <h2 className="max-w-xs text-2xl font-semibold tracking-tight text-white sm:text-3xl md:text-4xl">
          ...
          <MagneticButton className="w-fit rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[11px] uppercase tracking-[0.24em] text-white/55 transition hover:border-emerald-400/25 hover:text-emerald-100">
            Available now
          </MagneticButton>
        </div>

        <motion.div className="grid gap-4 sm:gap-5" variants={fadeUpContainer}>
          {projects.length === 0 ? (
            ...
            projects.map((project) => (
              <MagneticCard
                ...
                strength={10}
                variants={fadeUpItem}
                className="group rounded-3xl border border-white/10 bg-white/[0.04] p-4 transition duration-200 hover:border-emerald-400/25 hover:bg-white/[0.07] sm:p-5"
              >
